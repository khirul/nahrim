<template>
  <div>
    <div class="container" style="margin-bottom:70px;">
      <div class="row roof">Top-Up System</div>
...
export default {
  ...
  methods: {
    type() {
      ...
      if (this.tankType == "square") {
        this.tankheight_sqr = (
          this.$store.state.tankSize /
          (this.depth * this.width)
        ).toFixed(2);
        this.depthtopping_sqr = (
          this.$store.state.usagevol /
          1000 /
          (this.depth * this.width)
        ).toFixed(2);
        ...
        this.tankheight_cyl = (
          this.$store.state.tankSize /
          ((22 / 7) * (this.diameter / 2) ** 2)
        ).toFixed(2);
        this.depthtopping_cyl = (
          this.$store.state.usagevol /
          1000 /
          ((22 / 7) * (this.diameter / 2) ** 2)
        ).toFixed(2);
